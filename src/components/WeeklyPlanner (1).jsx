...
function RowRenderer({ row, week, onAdd, onEdit, onDelete, onSaveNotes }) {
  ...
  for (const day of DAYS) {
    ...
    if (cell.fixed) {
      if (cell.notesKey) {
        cells.push(
          <td key={day} rowSpan={rowSpan} style={{ ...styles.td, height: '100%' }}>
            <NotesCard
              cls={cell.cls}
              label={cell.fixed}
...
const styles = {
  ...
  notesCard: { borderRadius: 6, padding: '10px 8px', height: '100%', minHeight: 140, boxSizing: 'border-box', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start' },
